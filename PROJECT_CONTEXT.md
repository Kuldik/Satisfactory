...
- **Симуляция запущена (фаза 1).** `Engine.tick()` гоняет ECS-модель через
  ...
  JSON-паттерны и одиночные `.glb`-префабы из меню теперь оба попадают в снапшот
  симуляции; для уже поставленных до фикса `.glb` есть backfill из `builderPlaced`
  (`menuBuildingId + compositeId`). Логистика (ленты/трубы/рельсы/вагоны) пока
  исключена намеренно.
...
- Для handcrafted JSON-паттернов **не применять** `applyKenneyPipeLayFlatToRoot`: он нужен
  только трубам из меню (`pipe_mk*` / `pipe_junction`). Иначе space-station pipe/rail
  детали выглядят правильно в голограмме, но переворачиваются после установки.
- Конвейеры: в `default`-режиме нельзя авто-переводить L-угол в chord-прямую по
  «диагональности» курсора; это ломало кейс «превью L → после движения мыши стало
  прямой». Прямая линия должна жить в отдельном режиме `chord`.
